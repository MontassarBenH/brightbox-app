...
describe('FeedClient', () => {
  ...
  it('shows FAB buttons for contributor', async () => {
  ...
  it('hides FAB buttons for viewer', async () => {
    // @ts-expect-error - Mocking internal function
    ...
    await waitFor(() => {
      expect(screen.queryByTestId('fab-video-upload')).not.toBeInTheDocument()
    })
    expect(screen.queryByTestId('fab-create-post')).not.toBeInTheDocument()
  })
})
